...
const BudgetSummary = () => {
  ...
  // Calculate values based on current expenses
  ...
  // Update local state when totalBudget changes
  useState(() => {
    setNewBudget(totalBudget)
  }, [totalBudget])
  
  return (
    <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl shadow-lg p-6 lg:p-8 border border-green-100 print:bg-white print:border print:shadow-none">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-6 lg:mb-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 sm:space-x-4 print:hidden mt-4 lg:mt-0">
          {isEditing ? (
            <>
              <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 sm:space-x-3 w-full sm:w-auto">
                <div className="flex items-center space-x-3">
                  ...
                  <input
                    ...
                    className="px-4 py-2 border border-naija-green rounded-lg focus:outline-none focus:ring-2 focus:ring-naija-green focus:border-transparent w-full sm:w-auto"
                    min="1"
                  />
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6 mb-6 lg:mb-8">
        <div className="bg-white p-5 lg:p-6 rounded-lg shadow-sm border border-gray-200 print:border print:shadow-none">
          <div className="text-sm lg:text-base text-gray-500 mb-1 lg:mb-2 flex items-center">
            <FaChartBar className="mr-2" />
            Total Spent
          </div>
          <div className="text-2xl lg:text-3xl font-bold text-red-600">{formatAmount(totalSpent)}</div>
        ...
        <div className="bg-white p-5 lg:p-6 rounded-lg shadow-sm border border-gray-200 print:border print:shadow-none">
          <div className="text-sm lg:text-base text-gray-500 mb-1 lg:mb-2">Remaining</div>
        ...
        <div className="bg-white p-5 lg:p-6 rounded-lg shadow-sm border border-gray-200 md:col-span-2 lg:col-span-1 print:border print:shadow-none">
            ...
            {Object.entries(categories).map(([category, amount]) => (
              <div key={category} className="flex flex-col sm:flex-row sm:items-center justify-between">
                ...
                <div className="flex items-center space-x-4">
                  <div className="w-32 lg:w-48 bg-gray-200 rounded-full h-2.5 print:hidden">
                    <div 
                      className="h-2.5 rounded-full bg-green-500"
                      style={{ width: `${(amount / totalSpent) * 100}%` }}
                    ></div>
                  </div>
                  <span className="font-medium text-gray-900 text-sm lg:text-base">{formatAmount(amount)}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {isOverBudget && (
        <div className="mt-6 lg:mt-8 p-4 lg:p-5 bg-red-50 border border-red-200 rounded-lg print:border print:bg-red-50">
          <div className="flex items-center">
